...
class Go2_Data(Dataset):
    def __init__(self, data_root, config):
        ...
        preload_file = os.path.join(data_root, 'seq'+str(self.seq_len)+'_pred'+str(self.pred_len)+'.npy') #'_rp1'+str(self.rp1_close)+

        # dump to npy if no preload
        if  not os.path.exists(preload_file): # True: #
            preload_route = []
            preload_filename = []
            preload_rgb = []
            preload_dep_cld_xyz = []
            ...
            preload_xpos_ctrl = []
            preload_ypos_ctrl = []
            preload_xorient_ctrl = []
            preload_yorient_ctrl = []
            preload_velocity = []
            
            # list sub-directories in root 
            root_files = os.listdir(data_root)
            root_files.sort() #nanti sudah diacak oleh torch dataloader
            routes = [folder for folder in root_files if not os.path.isfile(os.path.join(data_root,folder))]
            for route in routes:
                route_dir = os.path.join(data_root, route)
                print(route_dir)
                ...
                for i in range(config.gap_bearing, len(files)-((self.seq_len-1)*config.seq_gap)-(self.pred_len*config.wp_gap)): #kurangi sesuai dengan jumlah sequence dan wp yang akan diprediksi
                    ...
                    for j in range(0, (self.seq_len*config.seq_gap), config.seq_gap):
                        filename = files[i+j][:-3] #hilangkan ekstensi filenya
                        rgbs.append(route_dir+"/camera/rgb/"+filename+"png")
                        dep_cld_xyzs.append(route_dir+"/camera/depth/cld_interp/"+filename+"npy")
                        rgb_segs.append(route_dir+"/camera/seg/map/"+filename+"png")
                        # pt_clouds.append(route_dir+"/point_cloud/"+filename+"npy")
                        pt_cld_pcds.append(route_dir+"/lidar/cld/"+filename+"pcd")
                        pt_cld_segs.append(route_dir+"/lidar/seg/"+filename+"npy")

                    #appendkan
                    preload_rgb.append(rgbs)
                    preload_dep_cld_xyz.append(dep_cld_xyzs)
                    preload_rgb_seg.append(rgb_segs)
                    ...
                    _, _, local_rob_heading = euler_from_quaternion(w=meta_current['local_orientation_xyzw'][3], 
                                                                    x=meta_current['local_orientation_xyzw'][0], 
                                                                    y=meta_current['local_orientation_xyzw'][1], 
                                                                    z=meta_current['local_orientation_xyzw'][2], rad=True)
                    loc_headings.append(local_rob_heading)
                    rob_curr_lat = meta_current['global_position_latlon'][0]
                    rob_curr_lon = meta_current['global_position_latlon'][1]
                    preload_lat.append(rob_curr_lat)
                    preload_lon.append(rob_curr_lon)
                    ...
                    with open(route_dir+"/meta/"+files[i+j-config.gap_bearing][:-3]+"yml", 'r') as prev_metafile:
                        prev_meta = yaml.safe_load(prev_metafile)
                    rob_prev_lat = prev_meta['global_position_latlon'][0]
                    rob_prev_lon = prev_meta['global_position_latlon'][1]
                    bearing_rob = latlon_to_yaw(rob_curr_lat, rob_curr_lon, rob_prev_lat, rob_prev_lon, offset=np.pi / 2.0)
                    bearing_rob_deg = np.degrees(bearing_rob) - 90
                    bearing_rob = np.radians(bearing_rob_deg)
                    preload_bearing.append(bearing_rob)
                    ...
                    #hitung velocity
                    rob_dLat_m = (rob_prev_lat-rob_curr_lat) * 40008000 / 360 #111320 #Y
                    rob_dLon_m = (rob_prev_lon-rob_curr_lon) * 40075000 * np.cos(np.radians(rob_curr_lat)) / 360 #X
                    movement = np.sqrt(rob_dLat_m**2 + rob_dLon_m**2)
                    velocity_ms = movement/(1/(config.hz*config.gap_bearing))
                    velocity = velocity_ms*3600/1000 #convert ke kmh
                    preload_velocity.append(velocity)
                    ...
                    preload_xpos_ctrl.append(meta_current['ctrl_lxy_rxy'][0])
                    preload_ypos_ctrl.append(meta_current['ctrl_lxy_rxy'][1])
                    preload_xorient_ctrl.append(meta_current['ctrl_lxy_rxy'][2])
                    preload_yorient_ctrl.append(meta_current['ctrl_lxy_rxy'][3])
                    ...
                    for r in range(2): #ada 2 route point
                        ...
                        dLat_m = (next_lat-rob_curr_lat) * 40008000 / 360 #111320 #Y
                        dLon_m = (next_lon-rob_curr_lon) * 40075000 * np.cos(np.radians(rob_curr_lat)) / 360 #X
                    ...
                    for k in range(1, self.pred_len+1):
                        filenamef = files[i+j + (k*config.wp_gap)] #  + (k*self.data_rate)future seconds, makanya dikali data rate
                        # meta
                        ...
                        _, _, flocal_rob_heading = euler_from_quaternion(w=meta_future['local_orientation_xyzw'][3], 
                                                                    x=meta_future['local_orientation_xyzw'][0], 
                                                                    y=meta_future['local_orientation_xyzw'][1], 
                                                                    z=meta_future['local_orientation_xyzw'][2], rad=True)
                        loc_headings.append(flocal_rob_heading)
            ...
            preload_dict['rgb_seg'] = preload_rgb_seg
            ...
            preload_dict['xpos_ctrl'] = preload_xpos_ctrl
            preload_dict['ypos_ctrl'] = preload_ypos_ctrl
            preload_dict['xorient_ctrl'] = preload_xorient_ctrl
            preload_dict['yorient_ctrl'] = preload_yorient_ctrl
        ...
        self.rgb_seg += preload_dict.item()['rgb_seg']
        ...
        self.xpos_ctrl += preload_dict.item()['xpos_ctrl']
        self.ypos_ctrl += preload_dict.item()['ypos_ctrl']
        self.xorient_ctrl += preload_dict.item()['xorient_ctrl']
        self.yorient_ctrl += preload_dict.item()['yorient_ctrl']
        self.velocity += preload_dict.item()['velocity']
        print("Preloading " + str(len(preload_dict.item()['filename'])) + " sequences from " + preload_file)
    ...
    def __getitem__(self, index):
        ...
        seq_dep_cloud_xyzs = self.dep_cld_xyz[index]
        seq_rgb_segs = self.rgb_seg[index]
        data['rgbs'] = []
        data['d_cld_xs'] = []
        # data['d_cld_ys'] = []
        data['d_cld_zs'] = []
        ...
        for i in range(0, self.seq_len):
            ...
            # print(i)
            # print(seq_rgbs)
            data['rgbs'].append(torch.from_numpy(np.array(resizecrop_matrix(cv2.imread(seq_rgbs[i]), WH_resized=[self.config.bev_w, self.config.bev_h], crop_HW=[self.config.bev_h, self.config.bev_w])).transpose(2,0,1)))
            dep_cloud = np.nan_to_num(resizecrop_matrix(np.load(seq_dep_cloud_xyzs[i])[:,:,0:3], WH_resized=[self.config.bev_w, self.config.bev_h], crop_HW=[self.config.bev_h, self.config.bev_w]).transpose(2,0,1), nan=0.0, posinf=39.99999, neginf=0.2) #min_d, max_d, -max_d, ambil xyz-nya saja 0:3, baca https://www.stereolabs.com/docs/depth-sensing/depth-settings/
            #dari dcloud
            #ini settingan untuk sl.COORDINATE_SYSTEM.RIGHT_HANDED_Z_UP_X_FWD
            pt_cloud_x = -1* torch.from_numpy(np.array(dep_cloud[1:2,:,:]))
            pt_cloud_z = torch.from_numpy(np.array(dep_cloud[0:1,:,:]))
            data['d_cld_xs'].append(pt_cloud_x)
            data['d_cld_zs'].append(pt_cloud_z)

            #data['d_cld_xs'].append(torch.from_numpy(np.array(dep_cloud[0:1,:,:])))
            # data['d_cld_ys'].append(torch.from_numpy(np.array(dep_cloud[1:2,:,:])))
            #data['d_cld_zs'].append(torch.from_numpy(np.array(dep_cloud[2:3,:,:])))
            depth = np.clip(np.sqrt(np.array(dep_cloud[0:1,:,:]**2 + dep_cloud[1:2,:,:]**2 + dep_cloud[2:3,:,:]**2))/39.99999, a_min=0.0, a_max=1.0) #dinormalisasi dan di clip 0-1
            data['depths'].append(torch.from_numpy(depth))
            #+1 karena class 0  dibuat untuk non_class atau none, penting untuk SDC!!!
            segmap = cv2.imread(seq_rgb_segs[i]) + 1
            data['rgb_segs'].append(torch.from_numpy(np.array(cls2one_hot(resizecrop_matrix(segmap, WH_resized=[self.config.bev_w, self.config.bev_h], crop_HW=[self.config.bev_h, self.config.bev_w]), n_class=self.config.n_class_cityscape))))
            ...
            if self.config.lidar_sensor == "mid360":
        ...
        # data['xpos_ctrl'] = self.xpos_ctrl[index]
        # data['ypos_ctrl'] = self.ypos_ctrl[index]
        # data['xorient_ctrl'] = self.xorient_ctrl[index]
        # data['yorient_ctrl'] = self.yorient_ctrl[index]
        #data['robot_ctrl'] = tuple(np.array([self.xpos_ctrl[index], self.ypos_ctrl[index], self.xorient_ctrl[index], self.yorient_ctrl[index]]))
        data['robot_ctrl'] = tuple(np.array([self.xpos_ctrl[index], self.ypos_ctrl[index], self.xorient_ctrl[index]]))
        # data['lr_velo'] = tuple(np.array([self.velocity_l[index], self.velocity_r[index]]))
        data['velo'] = self.velocity[index]
...
def gen_bev_front_seg_dep_numpy(self, ptx, pty, ptz, ptseg, bs=None):
    ...
    bev_seg = np.zeros((bsize, self.n_class_kitti, self.bev_h, self.bev_w))
    ...
    front_seg = np.zeros((bsize, self.n_class_kitti, self.front_h, self.front_w))
...
def colorize_seg(sem_map, colmap): #, add_bg=False
...
def colorize_logdepth(depth_map, invert=False):
    #inputnya sudah 0 - 1
    if invert:
        norm_dep = -1*depth_map[0][0] + 1
    else:
...
def colorize_depth(depth_map, invert=False):
    #inputnya sudah 0 - 1
    if invert:
        norm_dep = -1*depth_map[0][0] + 1
    else:
    ...
    coldepth = np.repeat(norm_dep[:, :, np.newaxis], 3, axis=2) * 255 #normalisasi ke 0 - 255
    return coldepth
...
def gen_bev_front_seg_infer(self, ptx, pty, ptz, ptseg):
    ...
    bev_seg = torch.zeros((1, self.n_class_kitti, self.bev_h, self.bev_w), dtype=self.dtype, device=self.gpu_device)
    ...
    coor_clsn = torch.unique(coorx[:, idx], dim=1).long() #tensor harus long supaya bisa digunakan sebagai index
    # front_seg = torch.zeros((bsize, self.n_class_kitti, self.front_h, self.front_w), dtype=self.dtype, device=self.gpu_device)
    front_seg = torch.zeros((1, self.n_class_kitti, self.front_h, self.front_w), dtype=self.dtype, device=self.gpu_device)
...
def gen_bev_front_seg_dep(self, ptx, pty, ptz, ptseg, bs=None, return_dep=True):
    ...
    if self.perspectives == 'fro':
        ...
        coor_clsn = torch.unique(coorx[:, idx], dim=1).long() #tensor harus long supaya bisa digunakan sebagai index
        bev_seg = torch.zeros((bsize, self.n_class_kitti, self.bev_h, self.bev_w), dtype=self.dtype, device=self.gpu_device)
        bev_seg[coor_clsn[0], coor_clsn[1], coor_clsn[2], coor_clsn[3]] = 1.0 #format axis dari NCHW
