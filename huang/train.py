...
def train(data_loader, model, config, writer, cur_epoch, optimizer, params_lw, optimizer_lw):
    ...
    for data in data_loader:
        ...
        total_loss = params_lw[0]*loss_seg + params_lw[1]*loss_ctrl
        ...
        if batch_ke == 0: #batch pertama, hitung loss awal
            ...
            if config.MGN:
                ...
                G1R = torch.autograd.grad(loss_ctrl, params[config.bottleneck[1]], retain_graph=True, create_graph=True)
                ...
                #dan rata2
                loss_hat_avg = (loss_seg_hat + loss_ctrl_hat) / len(config.loss_weights)
                ...
                C1 = (G_avg*inv_rate_ctrl).detach()**config.lw_alpha
...
def main():
    ...
    if not os.path.exists(config.logdir+"/trainval_log.csv"):
        ...
        #set optima lw baru
        latest_lw = [float(log_trainval['lw_ss'][-1:]), float(log_trainval['lw_ctrl'][-1:])]
    ...
    while True:
        ...
        log['lw_ctrl'].append(lws[1])
        log['elapsed_time'].append(elapsed_time)
        print('| t_total_l: %.4f | t_ss_l: %.4f | t_ctrl_l: %.4f |' % (train_log['t_total_l'], train_log['t_ss_l'], train_log['t_ctrl_l']))
        print('| v_total_l: %.4f | v_ss_l: %.4f | v_ctrl_l: %.4f |' % (val_log['v_total_l'], val_log['v_ss_l'], val_log['v_ctrl_l']))
