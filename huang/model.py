...
class huang(nn.Module): #
    ...
    def __init__(self, config):#n_fmap, n_class=[23,10], n_wp=5, in_channel_dim=[3,2], spatial_dim=[240, 320], gpu_device=None): 
        ...
        self.final_ss_f = ConvBlock(channel=[config.n_fmap_r50[0], config.n_class_cityscape], final=True)#, up=False)
        ...
        self.ctrl_branch = nn.ModuleList([nn.Sequential( 
            ...
            nn.Linear(config.control_huang[2], 3), #x pos, y pos, x orient, y orient
