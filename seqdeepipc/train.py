...
def train(data_loader, model, config, writer, cur_epoch, optimizer, params_lw, optimizer_lw):
    ...
    for data in data_loader:
        ...
        #forward pass
        pred_segs, pred_deps, pred_wp, pred_ctrl, _ = model(rgbs, d_cld_xs, d_cld_zs, rp1, rp2, gt_velocity, data['cmd'])#, seg_fronts[-1])
        ...
        loss_ctrl = l1l2loss(pred_ctrl, gt_robot_ctrl)
        total_loss = params_lw[0]*loss_seg + params_lw[1]*loss_dep + params_lw[2]*loss_wp + params_lw[3]*loss_ctrl
        ...
        if batch_ke == 0: #batch pertama, hitung loss awal
            ...
            loss_ctrl_0 = torch.clone(loss_ctrl)
            ...
            if config.MGN:
                ...
                G3R = torch.autograd.grad(loss_ctrl, params[config.bottleneck[1]], retain_graph=True, create_graph=True)
                ...
                loss_ctrl_hat = loss_ctrl / loss_ctrl_0
                #dan rata2
                loss_hat_avg = (loss_seg_hat + loss_dep_hat + loss_wp_hat + loss_ctrl_hat) / len(config.loss_weights)
                ...
                inv_rate_ctrl = loss_ctrl_hat / loss_hat_avg
                ...
                C3 = (G_avg*inv_rate_ctrl).detach()**config.lw_alpha
        ...
        score['ctrl_loss'].update(loss_ctrl.item())
        ...
        writer.add_scalar('t_ctrl_l', loss_ctrl.item(), cur_step)
...
def validate(data_loader, model, config, writer, cur_epoch):
    ...
    with torch.no_grad():
        ...
        for data in data_loader:
            ...
            loss_wp = l1l2loss(pred_wp, gt_waypoints)
            loss_ctrl = l1l2loss(pred_ctrl, gt_robot_ctrl)
            total_loss = loss_seg + loss_dep + loss_wp + loss_ctrl
            ...
            writer.add_scalar('v_ctrl_l', loss_ctrl.item(), cur_step)
...
def main():
    ...
    if not os.path.exists(config.logdir+"/trainval_log.csv"):
        ...
        #set optima lw baru
        latest_lw = [float(log_trainval['lw_ss'][-1:]), float(log_trainval['lw_de'][-1:]), float(log_trainval['lw_wp'][-1:]), float(log_trainval['lw_ctrl'][-1:])]
    ...
    while True:
        ...
        log['train_ctrl_loss'].append(train_log['t_ctrl_l'])
        log['val_ctrl_loss'].append(val_log['v_ctrl_l'])
        log['lgrad_loss'].append(lgrad)
        log['lw_ss'].append(lws[0])
        log['lw_de'].append(lws[1])
        log['lw_wp'].append(lws[2])
        log['lw_ctrl'].append(lws[3])
        log['elapsed_time'].append(elapsed_time)
        print('| t_total_l: %.4f | t_ss_l: %.4f | t_de_l: %.4f | t_wp_l: %.4f | t_ctrl_l: %.4f |' % (train_log['t_total_l'], train_log['t_ss_l'], train_log['t_de_l'], train_log['t_wp_l'], train_log['t_ctrl_l']))
        print('| v_total_l: %.4f | v_ss_l: %.4f | v_de_l: %.4f | v_wp_l: %.4f | v_ctrl_l: %.4f |' % (val_log['v_total_l'], val_log['v_ss_l'], val_log['v_de_l'], val_log['v_wp_l'], val_log['v_ctrl_l']))
