...
from model import rb15
from data import Go2_Data, colorize_seg, colorize_logdepth, colorize_depth
from log.rb15_seq1_pred5.config import GlobalConfig #pakai config.py yang dicopykan ke log
# import random
# random.seed(0)
# torch.manual_seed(0)
...
def test(data_loader, model, config):
    ...
    with torch.no_grad():
        ...
        for data in data_loader:
            ...
            pred_segs, pred_deps, pred_wp, pred_ctrl, pred_sdcs = model(rgbs, d_cld_xs, d_cld_zs, rp1, rp2, gt_velocity, data['cmd'])#, seg_fronts[-1])
            orien_ctrl, ypos_ctrl, xpos_ctrl, meta_pred = model.mlp_pid_control(pred_wp, pred_ctrl, velo)
            ...
            metric_dep = F.l1_loss(pred_deps[-1], deps[-1]) #ambil yang terakhir saja
            metric_wp = F.l1_loss(pred_wp, gt_waypoints)
            metric_ctrl = F.l1_loss(pred_ctrl, gt_robot_ctrl)
            total_metric = (1-metric_seg.item()) + metric_dep.item() + metric_wp.item() + metric_ctrl.item()
            ...
            score['de_metric'].update(metric_dep.item()) 
            ...
            log['test_de_metric'].append(metric_dep.item())
            ...
            pred_sdc = pred_sdcs[-1].cpu().detach().numpy() 
            imgx = colorize_seg(pred_seg, config.cityscapes_palette) #, add_bg=True
            imgx2 = colorize_seg(pred_sdc, config.cityscapes_palette)
            save_dir_seg = save_dir+'outputs/'+data['route'][-1]+'/pred_seg/'
            save_dir_sdc = save_dir+'outputs/'+data['route'][-1]+'/pred_sdc/'
            os.makedirs(save_dir_seg, exist_ok=True)
            os.makedirs(save_dir_sdc, exist_ok=True)
            #print(save_dir_seg+data['filename'][-1]+"png")
            cv2.imwrite(save_dir_seg+data['filename'][-1]+"png", imgx) #cetak predicted segmentation
            cv2.imwrite(save_dir_sdc+data['filename'][-1]+"png", imgx2) #cetak predicted sdc

            #visualisasi deph
            pred_dep = pred_deps[-1].cpu().detach().numpy() 
            depx = colorize_depth(pred_dep, invert=True)
            save_dir_dep = save_dir+'outputs/'+data['route'][-1]+'/pred_dep/'
            os.makedirs(save_dir_dep, exist_ok=True)
            cv2.imwrite(save_dir_dep+data['filename'][-1]+"png", depx)

            save_dir_meta = save_dir+'outputs/'+data['route'][-1]+'/meta/'
            os.makedirs(save_dir_meta, exist_ok=True)


            meta_pred['cmd'] = float(data['cmd'])
            meta_pred['rp1_pos_local'] = rp1[0].cpu().detach().numpy().tolist()
            meta_pred['rp2_pos_local'] = rp2[0].cpu().detach().numpy().tolist()
            meta_pred['rp1_pos_global'] = [float(data['lat_rp1']), float(data['lon_rp1'])]
            meta_pred['rp2_pos_global'] = [float(data['lat_rp2']), float(data['lon_rp2'])]
            meta_pred['robot_bearing'] = float(data['bearing_robot'])
            # meta_pred['velocity'] = velocity #km/h
            meta_pred['robot_pos_global'] = [float(data['lat_robot']), float(data['lon_robot'])]
            
            elapsed_time = time.time() - start_time #hitung elapsedtime
            meta_pred['fps'] = float(1/elapsed_time)
            meta_pred['model_fps'] = meta_pred['fps']
        ...
        log['test_de_metric'].append(np.mean(log['test_de_metric']))
        ...
        log['test_de_metric'].append(np.std(log['test_de_metric'][:-1]))
...
model = rb15(config).to(config.gpu_device, dtype=config.dtype)
